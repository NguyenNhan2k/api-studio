...
class CustomerService {
    ...
    async create(payload) {
        try {
            ...
            const [customers, created] = await db.Customers.findOrCreate({
                where: { [op.or]: { email: params.email, phone: params.phone } },
                defaults: params,
            });
            if (!created) {
                await this.response.setToastMsg('danger', 'Khách hàng đã tồn tại !', 1);
                return this.response;
            }
            await this.response.setToastMsg('success', 'Tạo thông tin khách hàng thành công!', 0);
            return this.response;
        } catch (error) {
            console.log(error);
            return response;
    ...
    async update(id, payload) {
        try {
            ...
            const user = await db.Customers.update(params, { where: { id: id } });
            if (!user[0]) {
    ...
    async destroy(id) {
        try {
            const user = await db.Customers.destroy({ where: { id: id } });
            if (!user) {
    ...
    async force(id) {
        try {
            const user = await db.Customers.destroy({ where: { id: id }, force: true });
            if (!user) {
                this.response.setMsg('Xóa thất bại!');
                return this.response;
            }
    ...
    async findOne(queries) {
        try {
            const user = await db.Customers.findOne(queries);
            if (!user) {
                return false;
            }
            return user.toJSON();
    ...
    async getOne(id) {
        try {
            ...
            let customer = await this.findOne(queries);
            if (!customer) {
                this.response.setMsg('Không tìm thấy khách hàng!');
                return this.response;
            }
            await this.response.pushResult({ customer });
            return this.response;
        } catch (error) {
            console.log(error);
            return this.response;
    ...
    async findAndCountAll(deleted = true) {
        const users = await db.Customers.findAndCountAll({
            where: {
                destroyTime: {
                    [op.not]: null,
                },
    ...
    async restore(id) {
        try {
            const retore = await db.Customers.restore({
                where: {
                    id,
                },
                raw: true,
    ...
    async destroyMutiple(arrId) {
        try {
            const restored = await db.Customers.destroy({
                where: {
                    id: arrId,
                },
                raw: true,
    ...
    async restoreMutiple(arrId) {
        try {
            const restored = await db.Customers.restore({
                where: {
                    id: arrId,
                },
                raw: true,
